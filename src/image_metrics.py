...
def fspecial_gaussian(size, sigma):
    ...
    サイズが偶数の場合、正の奇数に補正してからカーネルを生成します。
...
def PCQI(img1, img2, window=None, L=None):
    """
    Patch-based Contrast Quality Index (PCQI) を計算する関数。

    Parameters
    ----------
    img1 : ndarray
        比較対象の1つ目の画像 (2次元 numpy 配列)
    img2 : ndarray
        比較対象の2つ目の画像 (2次元 numpy 配列)
    window : ndarray, optional
        局所統計量計算用の窓（フィルタ）。指定しない場合、デフォルトは11×11のGaussianカーネル (σ=1.5) が使用される。
    L : int, optional
        画像のダイナミックレンジ。window が指定されない場合は256、指定される場合は255が用いられる。
    ...
    条件に合わない場合は、mpcqi と pcqi_map ともに -np.inf を返す。
    """
    # 入力画像を浮動小数点型の numpy 配列に変換
    ...
    if window is None and L is None:
        ...
        L = 256
    elif window is not None and L is None:
    ...
    pcqi_map *= (sigma12 + C) / (np.sqrt(sigma1_sq) * np.sqrt(sigma2_sq) + C)
    pcqi_map *= np.exp(-np.abs(mu1 - mu2) / L)
    
    mpcqi = np.mean(pcqi_map)
    return mpcqi, pcqi_map


def compute_pcqi(ref, target):
    """
    カラー画像の場合、グレースケールに変換してから PCQI を計算するラッパー関数。

    Parameters:
        ref (ndarray): 参照画像 (カラーの場合はHxWx3)
        target (ndarray): 対象画像 (カラーの場合はHxWx3)

    Returns:
        float: 計算された PCQI 値
    """
    if ref.ndim == 3:
        ref_gray = color.rgb2gray(ref) * 255
    else:
        ref_gray = ref
    if target.ndim == 3:
        target_gray = color.rgb2gray(target) * 255
    else:
        target_gray = target

    pcqi_val, _ = PCQI(ref_gray, target_gray)
    return pcqi_val
